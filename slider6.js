...
if (localStorage.getItem("slider6Data") == null) {
  localStorage.setItem("slider6Data", JSON.stringify(slider6Data));
}

function appendpro() {
  let data = JSON.parse(localStorage.getItem("slider6Data"));
  let mainDiv = document.getElementById("slider6_products");
...
function slider() {
  let sapn = document.getElementById("slider6").getElementsByTagName("span");
  let div = document.getElementById("slider6").getElementsByTagName("div");
  let l = 0;
  sapn[1].onclick = () => {
    l++; //for right scroll
    for (var i of div) {
